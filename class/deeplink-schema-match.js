...
class DeeplinkSchemaMatch {
  ...
  static navigationRouteFor(event, completionHandler, context = { wallets: [], saveToDisk: () => {}, addWallet: () => {} }) {
    ...
    if (isBothBitcoinAndLightning) {
      ...
      const urlObject = url.parse(event.url, true); // eslint-disable-line node/no-deprecated-api
      (async () => {
        if (urlObject.protocol === 'veleswallet:' || urlObject.protocol === 'lapp:' || urlObject.protocol === 'blue:') {
  ...
  /**
   * Extracts server from a deeplink like `veleswallet:setelectrumserver?server=electrum1.veles.network%3A443%3As`
  ...
  static getServerFromSetElectrumServerAction(url) {
    if (!url.startsWith('veleswallet:setelectrumserver') && !url.startsWith('setelectrumserver')) return false;
  ...
  /**
   * Extracts url from a deeplink like `veleswallet:setlndhuburl?url=https%3A%2F%2Flndhub.herokuapp.com`
   * returns FALSE if none found
   *
   * @param url {string}
   * @return {string|boolean}
  ...
  static getUrlFromSetLndhubUrlAction(url) {
    if (!url.startsWith('veleswallet:setlndhuburl') && !url.startsWith('setlndhuburl')) return false;
