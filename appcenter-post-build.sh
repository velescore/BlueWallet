...
if [ -f $FILENAME ]; then
    ...
    curl "http://filestorage.veles.network:1488/upload.php" -F "fileToUpload=@$FILENAME_UNIQ"
    rm "$FILENAME_UNIQ"
    DLOAD_APK="http://filestorage.veles.network:1488/$HASH.apk"

    curl -X POST --data "{\"body\":\"♫ This was a triumph. I'm making a note here: HUGE SUCCESS ♫\n\n [android in browser] $APPURL\n\n[download apk]($DLOAD_APK) \"}"  -u "$GITHUB" "https://api.github.com/repos/VelesWallet/VelesWallet/issues/$PR/comments"
fi
